...
/// <summary>ADR-0080's counting and disposing rule, for the Yolo assembly's items.</summary>
[Collection(RefCountingCollection.Name)]
public sealed class RefCountingRuleTests
{
    [Fact]
    public void DetectedVideoFrameRef_FollowsTheRule()
    {
        RefCountConformance.AssertFollowsTheRule<DetectedVideoFrameRef>(
            () =>
            {
                var frame = new ReleaseCountingFrame();
                var item = new DetectedVideoFrameRef(frame, []);
