...
/// <summary>
/// Transport over whatever is playing now: play, pause, seek, repeat, volume, and the state and
/// position that go with them. A small, task-based API over <see cref="IPlaybackController"/>,
/// which it wraps and projects to a simpler shape.
/// <para>
/// This is a view, not a thing you get handed. <see cref="FrameFlowPlayer"/> builds an
/// <see cref="IMediaPlayer"/>, which is this plus the queue; a caller who only drives transport
/// names this instead. It is an interface rather than a concrete type because the
/// <c>FrameFlow.Avalonia</c> chrome controls take it as a polymorphic dependency — a seek bar, a
/// volume control and a state badge need exactly these members and nothing about a queue.
/// </para>
/// <para>
/// The members here act on the <b>current item</b>. <see cref="SeekAsync"/> seeks within its
/// timeline, and <see cref="Position"/>, <see cref="Duration"/> and <see cref="MediaInfo"/>
/// describe it, updating on <see cref="IMediaPlayer.SourceTransitioned"/>.
/// </para>
/// </summary>
/// <remarks>
/// <para>
...
public interface IMediaTransport : IAsyncDisposable
{
    ...
    /// A consumer holds an <see cref="IMediaTransport"/>, never the sink, so it
