...
public partial class MainWindow : Window
{
    ...
    protected override async void OnLoaded(RoutedEventArgs e)
    {
        ...
        // A WinExe has no console, so the log goes to <repo>/logs/. There is no
        // flag for it: tools/FrameFlow.TestBench is where a run is driven and watched.
        _loggerFactory = LoggerFactory.Create(b =>
            b.SetMinimumLevel(LogLevel.Debug)
                .AddProvider(
                    new FileLoggerProvider(ExampleLogPaths.Resolve("avalonia-player.log"))
                )
        );
